🗺️Edco Core 👛 3.0.2 CE ©️ - 👒 RELEASE 👒  is now available from:
===============================
...
Is now available from:
  ...
  - CE release
    ...
    <https://github.com/cevap/edco/releases

  - Edco-coin release

    <https://github.com/edco-coin/edco/releases
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/cevap/edco/issues>

Recommended Update
==============

🗺️Edco Core 👛 3.0.2 CE ©️ - 👒 RELEASE 👒 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for xEDCO spends, automint calculation adjustments, and other various updates/fixes.

xEDCO spending requires this update.

How to Upgrade
==============

If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/Edco-Qt (on Mac) or edcod/edco-qt (on Linux).

Compatibility
==============

🗺️Edco Core 👛 is extensively tested on multiple operating systems using
...
🗺️Edco Core 👛 should also work on most other Unix-like systems but is not
...
xEDCO (_zerocoin for edco_)
-------------------------
We have now zerocoin into edco. More info about what it is and how to use it will follow in announcements and further release infos.

Snap/Snapcraft builds
----------------------------
Snapcraft is enabled and it can be simply installed by

`sudo snap install --edge edco`
...
Performance
-----------
Current source base is much faster and cleaner than edco's previous one. It uses all cpu's and there are 
...
Blockexplorer
-------------
We finaly have built in blockexplorer which works on all edco's networks.
...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each xEDCO mint operation (xEDCO spends which re-mint their change are also included in this). This functionality is controlled by the `-backupxion` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupxion=0` at the command-line when starting the client, or add `backupxion=0` to their `Educoin.conf` file.

xEDCO Automint Calculations
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in xEDCO mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

xEDCO Spending Fix
---------------------
The size of xEDCO spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

xEDCO Transaction Recovery
---------------------
Due to the aforementioned issue with xEDCO spending, users may find that their attempted spends are now conflicted and xEDCO balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the Edco address as it's first parameter and the passphrase as it's second parameter.
...
maintenanced then previous releases of 🗺️Edco Core 👛 which are based on obsolete code.
In this version everything is new, one big feature iz xEDCO which iz zerocoin for
edco. Port and other settings remain same. Please foolow our wiki for more information
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/edco/).
...
--------------------------------------
At this point you have two options, you can either use the automated script (found in https://github.com/cevap/edco/blob/master/contrib/gitian-build.sh, only works in Debian/Ubuntu) or you could manually do everything by following this guide. If you are using the automated script, then run it with the --setup command. Afterwards, run it with the --build command (example: contrib/gitian-build.sh -b signer 3.0.2). Otherwise ignore this.

Follow the instructions in https://github.com/cevap/edco/blob/master/doc/release-process.md in the bitcoin repository under 'Fetch and create inputs' to install sources which require manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache and offline git repositories' which will fetch the remaining files required for building offline.
...
# prebuild edco dependencies
cd ./gitian-builder
make -C ../edco/depends download SOURCES_PATH=`pwd`/cache/common
cd ..
```
...
./bin/gbuild --url edco=/home/gitianuser/edco,signature=/home/gitianuser/gitian.sigs --num-make 2 --memory 3000 --commit edco=v${VERSION} ../edco/contrib/gitian-descriptors/gitian-linux.yml

# Sign for linux
./bin/gsign --signer "$SIGNER" --release ${VERSION}-linux --destination ../gitian.sigs/ ../edco/contrib/gitian-descriptors/gitian-linux.yml
mv build/out/edco-*.tar.gz build/out/src/edco-*.tar.gz ../

# Build offline for windows
./bin/gbuild --url edco=/home/gitianuser/edco,signature=/home/gitianuser/gitian.sigs --num-make 2 --memory 3000 --commit edco=v${VERSION} ../edco/contrib/gitian-descriptors/gitian-win.yml

# Sign for windows
./bin/gsign --signer "$SIGNER" --release ${VERSION}-win-unsigned --destination ../gitian.sigs/ ../edco/contrib/gitian-descriptors/gitian-win.yml
mv build/out/edco-*-win-unsigned.tar.gz inputs/edco-win-unsigned.tar.gz
mv build/out/edco-*.zip build/out/edco-*.exe ../

# Build offline for mac os
./bin/gbuild --url edco=/home/gitianuser/edco,signature=/home/gitianuser/gitian.sigs --num-make 2 --memory 3000 --commit edco=v${VERSION} ../edco/contrib/gitian-descriptors/gitian-osx.yml

# Sign for macos
./bin/gsign --signer "$SIGNER" --release ${VERSION}-osx-unsigned --destination ../gitian.sigs/ ../edco/contrib/gitian-descriptors/gitian-osx.yml
mv build/out/edco-*-osx-unsigned.tar.gz inputs/edco-osx-unsigned.tar.gz
mv build/out/edco-*.tar.gz build/out/edco-*.dmg ../
cd ..
```
...
./bin/gbuild --num-make 2 --memory 3000 --commit edco=v${VERSION} ../edco/contrib/gitian-descriptors/gitian-linux.yml
...
./bin/gbuild --num-make 2 --memory 3000 --commit edco=v${VERSION} ../edco/contrib/gitian-descriptors/gitian-win.yml
...
./bin/gbuild --num-make 2 --memory 3000 --commit edco=v${VERSION} ../edco/contrib/gitian-descriptors/gitian-osx.yml
...
#### Linux
```
81c6c81ea4cf67f11d40b6215fdf3b7914577368b238eb4da2f7ff887f3d7b3b  edco-3.0.2-arm-linux-gnueabihf-debug.tar.gz
f3bf55bc9282a882410dabe793772501e85124322f8f30a1b0ae4f4684be0837  edco-3.0.2-arm-linux-gnueabihf.tar.gz
3b780fd5ba985847251a93a97c3aedad282e8cecb641c8873aa1591e3d84b420  edco-3.0.2-i686-pc-linux-gnu-debug.tar.gz
9483e8af1af7175efaf92c3efc99ec6ddee868e629be5269a9985abdbeade91c  edco-3.0.2-i686-pc-linux-gnu.tar.gz
a57d962fd75f97a4b73d9fb65f43611acf15c7d9139e840d455c421bf7253170  edco-3.0.2-x86_64-linux-gnu-debug.tar.gz
13ea2506bdf620d77d8316672adab68a0a5107ae9b1590a6ed5486f3b0509f63  edco-3.0.2-x86_64-linux-gnu.tar.gz
30e0390dc5d5f3bfbd91f340884ab75ca30056ff49c5831edc49fcae8c9413af  src/edco-3.0.2.tar.gz
76fa4b28ae291521be3ee12d22fa7d9b6085cafa0cc14c1c60b6b69f498d5a57  edco-linux-3.1-res.yml
```
#### windows
```
Generating report
e7e3a92a32a8dc924e64d19a276af11cfe74193cc5aad26bc7335e2e2e835a57  edco-3.0.2-win-unsigned.tar.gz
2a9b1199f03068c6ea18e5e25cd6736c15c7ddf9247ede67079bcd544c1426ad  edco-3.0.2-win32-debug.zip
624c4d36d9145efcaac2a9ed00368f2aac61631a9b75ebfc546926009e395db0  edco-3.0.2-win32-setup-unsigned.exe
49659943ab915444ab926bd392a34d9fb3397cf997f34e3deaa31f43bab185a7  edco-3.0.2-win32.zip
1a2ba3aac86fb44355b7b879f6c269d5556b1e302f3a54602c51101eedc3efe6  edco-3.0.2-win64-debug.zip
073f490607616680a5a5def73a1231885860aa047e526e7bf4694a4b88b11a56  edco-3.0.2-win64-setup-unsigned.exe
6f88c1bb4eec9b077c2e1ee60e9371b184d460a920e6170610f67dbc786e9faf  edco-3.0.2-win64.zip
679ac050e5555e097785b835f666de3d2e6b0e8af0dcefc7430ef3bfec386180  src/edco-3.0.2.tar.gz
eec21196d7d4abab4fb8c32882fed5157271161ecba4693a67d6ca407503e030  edco-win-3.1-res.yml
```
#### macos
```
7bc9149778661d03f5b26a97ff7dbbf8c9113d5198fe433e4e9b69cbb02f80f4  edco-3.0.2-osx-unsigned.dmg
2ae951d05b053790e0916db1e519fbf3f37258741ae67ffa15a8ddf455881b9b  edco-3.0.2-osx-unsigned.tar.gz
1b2f4dbcc02c423924a2dbbf2b813eca094fd796e1424a7478a14651d82524ae  edco-3.0.2-osx64.tar.gz
679ac050e5555e097785b835f666de3d2e6b0e8af0dcefc7430ef3bfec386180  src/edco-3.0.2.tar.gz
0be4184f68aaf67653b704e0bed30a83ede84fcea9c53ca39aa50c804acff614  edco-osx-3.1-res.yml
```