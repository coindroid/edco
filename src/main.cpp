...
#if defined(NDEBUG)
#error "EDCO cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uEDCO) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "Edco Signed Message:\n";
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : xEDCO with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : xEDCO spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: xEDCO spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight >= 1 && nHeight <= 10) {
        ...
        nSubsidy = 5000 * COIN;
    } else if (nHeight >= 16 && nHeight <= 115 ) {
        nSubsidy = 200 * COIN;
    } else {
        nSubsidy = 4 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from EDCO
...
void RecalculateXIONSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from xEDCO supply
...
bool RecalculateEDCOSupply(int nHeightStart)
{
...
/** FornaxA: Edco accepts PoS during PoW phase 
 *
 *   if (pindex->nHeight <= Params().LAST_POW_BLOCK() && block.IsProofOfStake())
 *       return state.DoS(100, error("ConnectBlock() : PoS period not active"),
 *           REJECT_INVALID, "PoS-early");
 */
/** cevap: Edco accepts PoS during PoW phase, PoW Ends with block 1000, PoS starts with 455, testnet 74 
 *
 */ 
/** cevap: Edco accepts PoS during PoW phase, PoW Ends with block 1000, PoS starts with 455, testnet 74 
 *
 ...
 */
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateEDCOSupply(Params().Zerocoin_StartHeight());
    }

    //Track xEDCO money supply in the block index
    if (!UpdateXIONSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new xEDCO supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s xEDCOSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert Edco to xEDCO
...
/*    if (block.GetBlockTime() > Params().Zerocoin_StartTime()) {
        ...
        // EDCOTor - disable reject block as our blocks are in version 8 since block 1
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent xEDCO spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of xEDCO serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d xEDCO mints and %d xEDCO spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // EDCO: We use certain sporks during IBD, so check to see if they are
