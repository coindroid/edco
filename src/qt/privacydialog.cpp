                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 xEDCO ought to be enough for anybody." - Bill Gates, 2017
    ui->xEDCOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelxEDCOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_9_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintxEDCO->setEnabled(false);
        ui->pushButtonMintxEDCO->setToolTip(tr("xEDCO is currently disabled due to maintenance."));

        ui->pushButtonSpendxEDCO->setEnabled(false);
        ui->pushButtonSpendxEDCO->setToolTip(tr("xEDCO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->xEDCOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintxEDCO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_9_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("xEDCO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_xEDCO, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " xEDCO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" xEDCO in ") +
...
void PrivacyDialog::on_pushButtonSpendxEDCO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_9_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("xEDCO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_xEDCO, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn xEDCO
        sendxEDCO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send xEDCO
    sendxEDCO();
}
...
void PrivacyDialog::on_pushButtonXIONControl_clicked()
{
    ...
    XIONControlDialog* xEDCOControl = new XIONControlDialog(this);
    xEDCOControl->setModel(walletModel);
    xEDCOControl->exec();
}

void PrivacyDialog::setXIONControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelxEDCOSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendxEDCO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Edco Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->xEDCOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as xEDCO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " Edco </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " xEDCO</b>";
    ...
    // use mints from xEDCO selector if applicable
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one xEDCO transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If xEDCO was spent successfully update the addressbook with the label
    ...
    ui->labelxEDCOSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("xEDCO Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " EDCO, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("xEDCO Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " xEDCO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" xEDCO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" xEDCO "));
    ui->labelxEDCOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));

    // Display AutoMint status
    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>xEDCO </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " xEDCO </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelxEDCOSyncStatus->setVisible(fShow);
}
