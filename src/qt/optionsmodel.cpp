...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::EDCO);
    ...
    if (!settings.contains("nAnonymizeEDCOAmount"))
        settings.setValue("nAnonymizeEDCOAmount", 1000);

    nAnonymizeEDCOAmount = settings.value("nAnonymizeEDCOAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeEDCOAmount"))
        SoftSetArg("-anonymizeionamount", settings.value("nAnonymizeEDCOAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in edco.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEDCOAmount:
            return QVariant(nAnonymizeEDCOAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEDCOAmount:
            nAnonymizeEDCOAmount = value.toInt();
            settings.setValue("nAnonymizeEDCOAmount", nAnonymizeEDCOAmount);
            emit anonymizeEDCOAmountChanged(nAnonymizeEDCOAmount);
            break;
        case CoinControlFeatures:
